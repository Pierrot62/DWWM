...
SELECT FLOOR(DATEDIFF(now(),`dateEntreeClient`)/365) AS anciennete, prenomClient,nomClient FROM clients ORDER BY anciennete
m. Affichez la quantité maximale achetée par un client.
SELECT MAX(`quantiteCommande`) FROM `commandes` INNER JOIN `clients` ON commandes.idCommande = clients.idClient;
